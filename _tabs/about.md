---
# the default layout is 'page'
icon: fas fa-info-circle
order: 7
---

Hi, I’m Andrew Filewich, a kinesiology student at the University of Calgary with a strong interest in exercise physiology, rehabilitation, and kinesiology education.

This website serves as my public academic portfolio and is centered on my honours thesis:

**Pre-Laboratory Instructional Videos in Experiential Learning: A Kinesiology Perspective**

My thesis explored whether short pre-laboratory instructional videos could support student preparation, procedural clarity, confidence, and skill development in undergraduate kinesiology laboratory courses.

## Research Focus

My interests include:

- experiential learning in kinesiology
- multimedia instruction and educational design
- exercise physiology and performance testing
- rehabilitation and applied movement science

## About This Site

This portfolio is designed to present my honours thesis in a more accessible web-based format. It includes:

- an overview of the research problem
- study methods and intervention design
- key findings and interpretation
- reflections on implications for kinesiology education

## Contact

You can find my work on GitHub here:

[github.com/afilewich](https://github.com/afilewich)
